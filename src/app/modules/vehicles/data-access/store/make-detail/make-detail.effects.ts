  ...
  loadMakeDetail$ = createEffect(() =>
    this.actions$.pipe(
      ...
      mergeMap(([action, makesState]) => {
        const makeName = makesState.makes.find((m: { makeId: number; makeName: string }) => m.makeId === action.makeId)?.makeName || '';
        ...
        return this.vehiclesApiService.getModelsForMake(makeName).pipe(
          ...
          catchError(error => of(MakeDetailActions.loadModelsFailure({ error: error.message || 'Error loading models' })))
        );
      })
    )
  );
    ...
    this.actions$.pipe(
      ...
      mergeMap(([action, makesState]) => {
        ...
        return this.vehiclesApiService.getVehicleTypesForMake(makeName).pipe(
          ...
          catchError(error => of(MakeDetailActions.loadTypesFailure({ error: error.message || 'Error loading types' })))
        );
      })
    )
  );
